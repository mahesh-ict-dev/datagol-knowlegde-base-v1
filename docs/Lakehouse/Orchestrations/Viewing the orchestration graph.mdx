---
sidebar_position: 6
tags: [Lakehouse, Connector, Pipeline, Orchestrations]
slug: viewing-the-orchestration-graph
---



import ImagePopup from '@site/src/components/ImagePopup';

The **Graphs** tab within an orchestration provides a clear visual representation of the pipeline's execution order, illustrating the sequence of stages. To view the orchestration graph do the following:

1.  On the DataGOL **Home** page, from the left navigation panel, click **Lakehouse** > **Orchestration**.
    
2.  In the **Orchestrations** page, click the link of an orchestration.
    
3.  Click the **Graphs** tab. The stages of the orchestration are listed in a graph. Click any stages to view more details of each pipeline execution.
    <ImagePopup src="/img/orchestration/image-20250512-041150.png" alt="viewing the orchestration graph" caption="viewing the orchestration graph" />